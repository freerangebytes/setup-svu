<!-- This file is generated. Do not edit directly. See .docs/README.md for details. -->

# svu Setup

Installs svu for use by other actions.

## Why Use This Action?

- **Multi-architecture support** – Automatically detects and downloads the correct binary for your runner's architecture (x86, x64, ARM, ARM64)
- **Immutable releases** – Downloads directly from [svu's official GitHub releases](https://github.com/caarlos0/svu/releases), ensuring you get authentic, unmodified binaries
- **Version pinning** – Specify exact versions for reproducible CI builds, avoiding unexpected changes from floating tags
- **Lightweight and focused** – Does one thing well: installs svu and gets out of your way

## Usage

```yaml
- uses: freerangebytes/setup-svu@v0.1.0
  with:
    # The version to install
    version: "3.3.0"
```

## Inputs

| Name | Description | Required | Default |
| ---- | ----------- | -------- | ------- |
| `version` | The version to install | `true` | `3.3.0` |

## Outputs

| Name | Description |
| ---- | ----------- |
| `version` | The version of svu installed |

## Contributing

### Development Environment

This project includes a [devcontainer](https://containers.dev/) configuration for a consistent development experience. You can use it with:

- [GitHub Codespaces](https://github.com/features/codespaces)
- [VS Code Dev Containers](https://code.visualstudio.com/docs/devcontainers/containers)
- Any IDE that supports the devcontainer specification

The devcontainer comes pre-configured with all required tools and recommended VS Code extensions.

### Commit Messages

This project uses [commitlint](https://commitlint.js.org/) with the [Conventional Commits](https://www.conventionalcommits.org/) specification. All commit messages must follow the format:

```
<type>[optional scope]: <description>

[optional body]

[optional footer(s)]
```

Common types include: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`.

For more information, see the [commitlint documentation](https://github.com/conventional-changelog/commitlint/#what-is-commitlint).

## License

MIT
